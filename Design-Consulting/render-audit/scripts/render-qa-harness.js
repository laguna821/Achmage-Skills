...
(function () {
  ...
  var SCRIM = {
    'sc-cover':      { c: [4, 52, 58], a: 0.74, op: 1.00, mul: 1 },
    'sc-tone':       { c: [4, 52, 58], a: 0.56, op: 1.00, mul: 1 },
    'sc-neon':       { c: [4, 52, 58], a: 0.62, op: 1.00, mul: 1 },
    'sc-horizon':    { c: [4, 52, 58], a: 0.50, op: 1.00, mul: 1, horizon: 1 },
    'sc-wash':       { c: null,        a: 0.82, op: 0.34, mul: 0, blur: 8,
                       dark: { c: [34, 56, 61], a: 0.86, op: 1.00, mul: 1, blur: 6 } },
    'sc-wash-012':   { c: null,        a: 0.78, op: 0.12, mul: 0, blur: 10,
                       dark: { c: [22, 38, 42], a: 0.78, op: 1.00, mul: 1, blur: 8 } },
    'sc-behind':     { c: null,        a: 0.80, op: 0.16, mul: 0,
                       dark: { c: [34, 56, 61], a: 0.84, op: 0.85, mul: 1 } },
    'sc-luma':       { c: null,        a: 0.84, op: 0.40, mul: 0, lum: 1,
                       dark: { c: null, a: 0.70, op: 1.00, mul: 0, lum: 1 } },
    /* 밴드-변주 키 'cls@band--x' — 같은 기법이라도 밴드 지면이 다르면 다크
     * 스크림이 갈린다 (실측: attacker(mid)/case-b(light) 가 α 하나에 묶이면
     * 한쪽은 관측성, 한쪽은 대비가 무너진다). sceneOf 가 자동 해석. */
    'sc-luma@band--light': { c: null,  a: 0.84, op: 0.40, mul: 0, lum: 1,
                       dark: { c: null, a: 0.76, op: 1.00, mul: 0, lum: 1 } },
    'sc-cut':        { c: null,        a: 0.75, op: 0.62, mul: 0,
                       dark: { c: null, a: 0.62, op: 0.90, mul: 0 } },
    'sc-cardwindow': { c: null,        a: 0.74, op: 0.50, mul: 0,
                       dark: { c: null, a: 0.70, op: 0.80, mul: 0 } }
  };
  ...
  window.RQ1 = function () {
    ...
    var edges = [];
    secs.forEach(function (s) {
      var w = s.querySelector(':scope > .wrap'); if (!w) return;
  ...
  window.RQ2 = async function () {
    ...
    for (var si = 0; si < secs.length; si++) {
      var sec = secs[si];
      var cls = sceneOf(sec);
      var band = bgRGB(sec);  // F55: parseRGB 는 alpha=0(배경 미지정 = 지면 위임)을 무시해 흑 [0,0,0] 으로 오판 — 라이트 거짓 FAIL·다크 거짓 관대 (실전 4호 실측). 조상 워크로 실배경을 취한다.
